...
type srgbTest struct {
	...
	instr    string
	maxError float64
	fn       func(in *WideTileBuffer, out *[wideTileWidth][stripHeight][4]uint8, unpremul bool)
	disabled bool
}

func linearRgbaF32ToSrgbU8_Reference(
	in *WideTileBuffer,
	...
	for x := range in {
		for y, px := range &in[x] {
			...
			for k, v := range px[:3] {
				if v < 0.0031308 {
					out[x][y][k] = uint8(0.5 + (12.92 * v * 255))
				} else {
					s := 1.055*(math.Pow(float64(v), 1.0/2.4)) - 0.055
					out[x][y][k] = uint8(0.5 + s*255)
				}
			}
			out[x][y][3] = uint8(0.5 + 255*px[3])
		}
	}
}

func linearRgbaF32ToSrgbF32Reference(
	...
	for _, tt := range linearRgbaF32ToSrgbU8Tests {
		for _, unpremul := range []bool{false, true} {
			t.Run(fmt.Sprintf("name=%s/unpremul=%t/instr=%s", tt.name, unpremul, tt.instr), func(t *testing.T) {
				...
				var out [wideTileWidth][stripHeight][4]uint8
