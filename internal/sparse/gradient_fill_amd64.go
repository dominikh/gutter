...
//go:build goexperiment.simd && !noasm
...
func (gf *gradientFiller) fillAVX2(dst Pixels) {
	switch kind := gf.gradient.kind.(type) {
	case encodedLinearGradient:
		gf.fillLinearAVX2(dst, kind)
	case encodedRadialGradient:
		gf.fillRadialAVX2(dst, kind)
	case encodedStripGradient:
		gf.fillStripAVX2(dst, kind)
	case encodedFocalGradient:
		gf.fillFocalAVX2(dst, kind)
	case encodedSweepGradient:
		gf.fillSweepAVX2(dst, kind)
	default:
...
// applyExtendAVX2 applies the gradient extend mode to a wide vector of t values.
func applyExtendAVX2(
	t Float32x8,
...
func (gf *gradientFiller) fillLinearAVX2(
	dst Pixels,
	...
	for x := 0; x < width; x += 2 {
		t := applyExtendAVX2(posX8, gf.gradient.extend)
		t.Store((*[8]float32)(unsafe.Pointer(&tBuf[x])))
		posX8 = posX8.Add(twoXAdvXVec)
	}

	runGradientAVX2(gf.gradient, dst, &tBuf, nil)
}

func (gf *gradientFiller) fillRadialAVX2(
	dst Pixels,
	...
	for x := 0; x < width; x += 2 {
		...
		t = applyExtendAVX2(t, gf.gradient.extend)
		t.Store((*[8]float32)(unsafe.Pointer(&tBuf[x])))
		posX8 = posX8.Add(twoXAdvXVec)
...
func (gf *gradientFiller) fillStripAVX2(
	dst Pixels,
	...
	runGradientAVX2(gf.gradient, dst, &tBuf, &maskBuf)
}

func (gf *gradientFiller) fillFocalAVX2(
	dst Pixels,
...
func (gf *gradientFiller) fillSweepAVX2(
	dst Pixels,
...
// runGradientAVX2 takes t values and a mask and fills dst with the gradient g.
func runGradientAVX2(
	g *encodedGradient,
	dst Pixels,
	tBuf *[wideTileWidth][stripHeight]float32,
	masks *[wideTileWidth / 2]uint8,
) {
	width := dst.width()
	if masks != nil {
		masksSlice := masks[:]
	...
	if len(g.ranges) <= maxShuffleGradientRanges {
		runGradientShuffleAVX2(
			dst.plane(0),
			dst.plane(1),
			dst.plane(2),
			dst.plane(3),
	...
	if masks != nil {
		...
		for i := range width / 2 {
			...
			case 255:
				// Nothing to do
			default:
				expandedMask := Int32x4{}.
					SetElem(0, int32(b)).
					Broadcast1To8().
					And(bitIsolateMask).
					NotEqual(bitIsolateMask)
				for ch := range 4 {
					Float32x8{}.
						StoreMasked(safeish.Cast[*[8]float32](&dst.plane(ch)[i*2]), expandedMask)
				}
			}
		}
		if width%2 != 0 {
			...
			default:
				expandedMask := Int32x4{}.
					SetElem(0, int32(b)).
					Broadcast1To4().
					And(bitIsolateMask.GetLo()).
					NotEqual(bitIsolateMask.GetLo())
				for ch := range 4 {
...
func runGradientShuffleAVX2(
	dst0_ [][stripHeight]float32,
	dst1_ [][stripHeight]float32,
	dst2_ [][stripHeight]float32,
	dst3_ [][stripHeight]float32,
	tBuf_ *[256][stripHeight]float32,
	...
	dst0 := safeish.SliceCast[[]float32](dst0_)
	dst1 := safeish.SliceCast[[]float32](dst1_)
	dst2 := safeish.SliceCast[[]float32](dst2_)
	dst3 := safeish.SliceCast[[]float32](dst3_)
	tBuf := safeish.SliceCast[[]float32](tBuf_[:])

	_ = dst0[widthFloats-1]
	_ = dst1[widthFloats-1]
	_ = dst2[widthFloats-1]
	_ = dst3[widthFloats-1]

	if nMinus1 >= maxShuffleGradientRanges {
		// Eliminate bounds check on sr.x1[threshIdx]
		panic("unreachable")
	}

	offset := 0
	for ; offset < widthFloats; offset += 8 {
		// OPT(dh): this has a bounds check as we can't prove that there are at
		// least 8 items left.
		//
		// tBuf is always long enough to load 8 values from, no need for
		// SlicePart.
		t := LoadFloat32x8Slice(tBuf[offset:])

		idx := Uint32x8{}
		for threshIdx := range nMinus1 {
			thresh := BroadcastFloat32x8(sr.x1[threshIdx])
			incs := t.GreaterEqual(thresh)
			// incs contains -1s where t >= threshold. By subtracting incs from
			// idx, we increment idx. When the loop is done, idx contains the
			// indices of the ranges corresponding to our t values.
			idx = idx.Sub(incs.ToInt32x8().AsUint32x8())
		}

		{
			s := scaleR.Permute(idx)
			b := biasR.Permute(idx)
			s = s.MulAdd(t, b)
			s.StoreSlicePart(dst0[offset:])
		}
		{
			s := scaleG.Permute(idx)
			b := biasG.Permute(idx)
			s = s.MulAdd(t, b)
			s.StoreSlicePart(dst1[offset:])
		}
		{
			s := scaleB.Permute(idx)
			b := biasB.Permute(idx)
			s = s.MulAdd(t, b)
			s.StoreSlicePart(dst2[offset:])
		}
		{
			s := scaleA.Permute(idx)
			b := biasA.Permute(idx)
			s = s.MulAdd(t, b)
			s.StoreSlicePart(dst3[offset:])
		}
	}
